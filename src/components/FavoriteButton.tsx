...
export function FavoriteButton({ word, reading, english, partOfSpeech, isFavorited, onToggle }: FavoriteButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-6 h-6 rounded-full flex items-center justify-center hover:bg-yellow-100 active:scale-90 transition-all"
      title={favorited ? 'Remove from favorites' : 'Add to favorites'}
      disabled={loading}
    >
      {loading ? (
        <span className="w-3.5 h-3.5 block animate-spin rounded-full border-2 border-yellow-300 border-t-yellow-600"></span>
      ) : (
        <svg
