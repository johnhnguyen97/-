...
export function CalendarDetailPopover({ type, data, onClose }: CalendarDetailPopoverProps) {
    ...
    return (
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4">
          <div className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5 sm:mb-2">
            Meaning
          </div>
        ...
        <div className="flex gap-1.5 sm:gap-2 flex-wrap justify-center">
          {word.partOfSpeech && (
            <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs sm:text-sm rounded-full">
              {word.partOfSpeech}
            </span>
          )}
          {word.jlptLevel && (
            <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-xs sm:text-sm rounded-full font-bold">
              {word.jlptLevel}
            </span>
          )}
          {word.isLearned && (
            <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs sm:text-sm rounded-full">
              ✓ Learned
            </span>
          )}
        </div>
    ...
    return (
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="text-center">
          <div className="relative inline-block bg-gray-50 dark:bg-gray-800 rounded-xl sm:rounded-2xl p-3 sm:p-4">
            {svgContent ? (
              <div
                ref={svgContainerRef}
                className="w-36 h-36 sm:w-48 sm:h-48 mx-auto flex items-center justify-center [&_svg]:w-full [&_svg]:h-full [&_svg]:max-w-[140px] [&_svg]:max-h-[140px] sm:[&_svg]:max-w-[180px] sm:[&_svg]:max-h-[180px]"
              />
            ) : loadingSvg ? (
              <div className="w-36 h-36 sm:w-48 sm:h-48 mx-auto flex items-center justify-center">
                <div className="animate-spin h-8 w-8 sm:h-10 sm:w-10 border-3 border-purple-500 border-t-transparent rounded-full" />
              </div>
            ) : (
              <div className="text-6xl sm:text-8xl font-bold text-gray-900 dark:text-white py-3 sm:py-4">
                {kanji.kanji}
              </div>
            )}
          </div>
          ...
          {svgContent && (
            <div className="mt-3 sm:mt-4 flex gap-2 justify-center">
              <button
                ...
                className={`px-3 sm:px-4 py-2 text-xs sm:text-sm rounded-full transition-all flex items-center gap-1.5 sm:gap-2 touch-manipulation active:scale-95 ${
                  isAnimating
        ...
        <div className="flex gap-1.5 sm:gap-2 flex-wrap justify-center">
          {kanji.strokeCount && (
            <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs sm:text-sm rounded-full font-medium">
  ...
  const renderHolidayContent = () => {
    ...
    return (
      <div className="space-y-3 sm:space-y-4">
        ...
        {holiday.description && (
          <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg sm:rounded-xl p-3 sm:p-4">
        ...
        {holiday.traditions && holiday.traditions.length > 0 && (
          <div>
            <div className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 sm:mb-3">
              Traditions
            </div>
            <div className="flex flex-wrap gap-1.5 sm:gap-2 justify-center">
              {holiday.traditions.map((tradition, i) => (
                <span
                  key={i}
                  className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs sm:text-sm rounded-full"
                >
                  {tradition}
                </span>
              ))}
  ...
  const popoverContent = (
    <>
      ...
      {/* Centered Modal - Mobile Optimized */}
      <div
        className={`fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4 pointer-events-none transition-all duration-300 ${
          isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90'
        }`}
      >
        <div
          ref={popoverRef}
          className="bg-white dark:bg-gray-900 rounded-xl sm:rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md max-h-[90vh] sm:max-h-[85vh] overflow-hidden pointer-events-auto"
        >
          {/* Header */}
          <div className={`bg-gradient-to-r ${getTitleColor()} px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between`}>
            <h3 className="text-white font-bold text-base sm:text-lg">{getTitle()}</h3>
            <button
              onClick={handleClose}
              className="p-2 text-white/80 hover:text-white hover:bg-white/20 rounded-lg transition-colors touch-manipulation"
            >
          ...
          {/* Content */}
          <div className="p-4 sm:p-5 overflow-y-auto max-h-[calc(90vh-70px)] sm:max-h-[calc(85vh-80px)]">
