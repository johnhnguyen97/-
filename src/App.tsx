...
function AppContent() {
  ...
  // Track if game is active (to hide input)
  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    // Trigger game visible animation after a small delay
  ...
  // Reset to input mode - clear everything and show input again
      ...
      <div className="flex">
        ...
        <div className={`flex-1 py-8 px-4 transition-all duration-300 ${hasSentences && sidebarOpen ? 'lg:mr-80' : ''}`}>
          <div className="max-w-4xl mx-auto">
            {/* Header - Always visible but smaller when game active */}
            <header className={`text-center relative transition-all duration-500 ${gameActive ? 'mb-4' : 'mb-8'}`}>
              <div className={`transition-all duration-500 ${gameActive ? 'scale-75 origin-top' : ''}`}>
                <h1 className="text-4xl font-bold text-gray-800 mb-2">
                  <span className="bg-gradient-to-r from-amber-600 to-orange-500 bg-clip-text text-transparent">語順</span>
                  {' '}
                  <span className="text-2xl text-gray-500">(Gojun)</span>
                </h1>
                {!gameActive && (
              ...
              {!hasApiKey && !gameActive && (
                <div className="mt-4 p-3 bg-amber-100 border border-amber-300 rounded-lg text-amber-800 text-sm animate-fadeInUp">
            ...
            {globalError && (
              <div className="mb-6 p-4 bg-red-100 border border-red-300 rounded-lg text-red-700 animate-shake">
                {globalError}
              </div>
            )}

            {/* Input Section - Hidden when game is active */}
            <div className={`transition-all duration-500 overflow-hidden ${
              gameActive ? 'max-h-0 opacity-0 mb-0' : 'max-h-[500px] opacity-100 mb-8'
            }`}>
              <SentenceInput onSubmit={handleTextSubmit} isLoading={isLoadingAll} />

              {/* Instructions when no sentence */}
              {!gameActive && (
                <div className="mt-6 text-center p-8 bg-white/60 backdrop-blur-sm rounded-2xl border-2 border-dashed border-amber-200 animate-fadeInUp">
            ...
            {/* Game Section - Appears with animation */}
            <div className={`transition-all duration-500 ${
              gameActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'
            }`}>
              {/* Progress Overview (when multiple sentences) */}
              {sentences.length > 1 && (
                <div className={`mb-6 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-amber-200 flex items-center justify-between shadow-sm transition-all duration-300 ${
                  gameVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
                }`}>
                      ...
                      {sentences.map((s, i) => (
                        <button
                          ...
                          className={`w-3 h-3 rounded-full transition-all ${
                            s.isComplete ? 'bg-green-500' : s.isLoading ? 'bg-gray-300 animate-pulse' : 'bg-amber-300'
              ...
              <div className="space-y-8">
                {sentences.map((sentence, index) => (
                  ...
                  >
                    ...
                    {sentences.length > 1 && (
                      <div className={`absolute -left-4 -top-2 w-8 h-8 rounded-full flex items-center justify-center font-bold text-white z-10 transition-all shadow-lg ${
                    ...
                    {sentence.isLoading ? (
                      <div className="p-8 bg-white/80 backdrop-blur-sm rounded-2xl border border-amber-200 text-center shadow-sm">
                        ...
                        <p className="text-amber-600 font-medium mt-1">"{sentence.original}"</p>
                      </div>
                    ) : sentence.error ? (
                      ...
                      <div className={`transition-all ${sentence.isComplete ? 'ring-2 ring-green-500 ring-offset-2 rounded-2xl' : index === activeSentenceIndex ? 'ring-2 ring-amber-300 ring-offset-2 rounded-2xl' : ''}`}>
                        <SentenceDisplay
                        ...
                        <div className="flex justify-center gap-3 mt-6">
                          <button
                            ...
                            className="px-5 py-2.5 text-gray-600 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-all hover:shadow-md font-medium"
                          >
                            ← New Sentence
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleReset(index); }}
                            className="px-5 py-2.5 text-amber-600 bg-amber-50 border border-amber-200 rounded-xl hover:bg-amber-100 transition-all hover:shadow-md font-medium"
                          >
                            Reset
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleShowAnswers(index); }}
                            className="px-5 py-2.5 text-white bg-gradient-to-r from-amber-500 to-orange-500 rounded-xl hover:shadow-lg transition-all font-medium"
                          >
                            Show Answers
                          </button>
                        </div>
                        ...
                        {sentence.isComplete && !sentence.showAnswers && (
                          <div className="mt-4 p-4 bg-gradient-to-r from-green-400 to-emerald-500 rounded-xl text-center text-white animate-scaleIn">
              ...
              {totalCount > 0 && completedCount === totalCount && !sentences.some(s => s.showAnswers) && (
                <div className="mt-8 p-6 bg-gradient-to-r from-green-400 to-emerald-500 rounded-2xl text-center text-white shadow-xl animate-scaleIn">
        ...
        {hasSentences && sidebarOpen && allGrammarData.length > 0 && (
          <div className="hidden lg:block fixed right-0 top-0 h-screen w-80 bg-white/95 backdrop-blur-md border-l border-gray-200 overflow-y-auto shadow-xl">
            <GrammarSidebar
              grammarData={allGrammarData}
              activeSentenceIndex={activeSentenceIndex}
              onSelectSentence={setActiveSentenceIndex}
